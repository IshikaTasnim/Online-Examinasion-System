...
<?php
  if (isset($_POST['create'])){
    ...
    $semester=$semester.'-'.$year;
     $check = $db->query("SELECT * FROM semester where sem_name='$semester'");
    if(empty($year)  ) {
      ...
      $insert = $db->query("INSERT semester (sem_name) VALUES ('$semester')");
  ...
  $id = protect($_GET['id']);
  $query = $db->query("delete FROM semester WHERE sem_id='$id'");
?>
...
<div class="container fluid">
  <div class="panel-heading">
    <h1 class="page-title" align="center">Create New Semester</h1>
  </div>
  <div class="panel-body">
    <form class="form-horizontal" action="" method="post">
      <div class="row">
  ...
  <div>
    ...
    <table class="table" align="center">
       ...
       <?php
       $i=1;
            $query = $db->query("SELECT * FROM semester ORDER BY `sem_id` DESC");
            if($query->num_rows>0) {
              $msg="Are you sure to delete this id?";
              while($row = $query->fetch_assoc()) {
                echo'<tr><td>'.$i.'</td><td>'.$row['sem_name'].'</td><td><a onclick="return confirm()" href="create_new_semester.php?action=delete&id='.$row['sem_id'].'">Delete</a></td></tr>';
              $i++;}
            } else {
