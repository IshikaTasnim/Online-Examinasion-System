...
<?php if(is_teacher_Loggedin()):?>
 <div class="">
  <form method="POST" action="">
 	...
 	<h1 align="center">All Question List</h1>
 	<div class="row">
       <div class="col-md-6">
  ...
  ?>
 	<table class="table">
 		...
 		<?php
              $i=1;
                $query = $db->query("select * from question ,subject inner join exam on exam.sub_id=subject.sub_id where subject.sub_id='$sub_id'");
                if($query->num_rows>0) {
                  while($row = $query->fetch_assoc()) {
                    echo '<tr><td>'.$i.'</td><td>'.$row['q_name'].'</td><td>'.$row['sub_name'].'</td><td><a href="edit_question.php?id='.$row['q_id'].'">Edit</a> | <a href="question_list.php?id='.$row['q_id'].'">Delete</a></td></tr>';
                    $i++;
                  }
                } else {
